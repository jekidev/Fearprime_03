# Debug / methods — index

Denne mappe indeholder **metode og audit**, ikke primær evidens.

## Metodestandarder
- [Research Rules](RESEARCH_RULES.md)
- [Systematic Search Protocol](SYSTEMATIC_SEARCH_PROTOCOL.md)
- [Risk of Bias Template](RISK_OF_BIAS_TEMPLATE.md)
- [Effect-size Extraction Standard](EFFECT_SIZE_EXTRACTION_STANDARD.md)
- [Certainty Framework](CERTAINTY_FRAMEWORK.md)
- [Participant Overlap Register](PARTICIPANT_OVERLAP_REGISTER.md)

## Aktuel status
- [Coverage Plan](COVERAGE_PLAN.md) — domænedækning + næste forskningsarbejde
- [v1.0 Gap Audit](FEARPRIME_V1_GAP_AUDIT.md) — readiness mod v1.0

Disse to filer har forskellige roller og må ikke udvikle parallelle changelogs.

## Claim profiles
- [Reconsolidation Certainty Profile](RECONSOLIDATION_CERTAINTY_PROFILE.md)
- [Candidate Certainty Profile](CANDIDATE_CERTAINTY_PROFILE.md)

## Reconsolidation audit
- [Rolling Search Log](RECONSOLIDATION_SEARCH_LOG.md)
- [Dated Search 2026-09-18](RECONSOLIDATION_SEARCH_2026-09-18.md)
- [PubMed Export Status](PUBMED_RECONSOLIDATION_EXPORT_2026-09-18.md)
- [Effect-size/RoB Audit](RECONSOLIDATION_EFFECT_SIZE_ROB_AUDIT.md)

## Andre søgelogs
Butyrat/Latuda, core drugs, theory, translation og TRE logs bevares som historisk provenance.

## Preregistration
- [Interoceptive Augmentation Preregistration](INTEROCEPTIVE_AUGMENTATION_PREREGISTRATION.md)

## Struktur / release
- [Structure Audit 2026-09-18](STRUCTURE_AUDIT_2026-09-18.md)
- [Release Checklist](RELEASE_CHECKLIST.md)

## Archive
[ARCHIVE/](ARCHIVE/) indeholder snapshots af tidligere store indeksfiler. De er historik og skal ikke bruges som aktiv evidenssyntese.
