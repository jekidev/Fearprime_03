...
```mermaid
flowchart TD
  E["Erfaring og kontekst"] --> L["Aktiveret læring"]
  B["Biologiske betingelser"] --> P["Mulighed for plasticitet"]
  L --> S["Synaptiske ændringer"]
  P --> S
  S --> F["Fastholdelse og senere genkaldelse"]
  K["Kontekst ved senere test"] --> F
  F --> U["Målt forventning og handling"]
```
